---
node: Korean traditional
detail: 104 tiles, no bamboo
family: korean
confidence: placeholder
parent: Chinese classical
chart_id: korean-traditional
canonical_source: sloperama-faq-2b
chart_refs: [1, 7]
---

# Korean traditional

*104 tiles, no bamboo*

**Lineage** · [Proto-mahjong](../../) → [Chinese classical](../) → **Korean traditional**

**Family** · Korean

**Confidence** · `placeholder` — no source establishes the descent at all; the parent link is a guess ([vocabulary](../../../_dev/CONVENTIONS.md#provenance-vocabulary))

## Caveat from the source chart

> Drawn with a dashed connector in the source chart. No source establishes how the 104-tile Korean game descends from anything; the parent link is a placeholder.

## Sources the chart cites for this node

| # | Source | URL | Role |
| --- | --- | --- | --- |
| [1](../../../REFERENCES.md#ref-1) | Sloper, Tom. "FAQ 2b: Identifying a Mah-Jongg Variant." Sloperama. | <https://www.sloperama.com/mjfaq/mjfaq02b.html> | **canonical** |
| [7](../../../REFERENCES.md#ref-7) | Mahjong Wiki. "Family Tree and Timeline." mahjong.wikidot.com. | <http://mahjong.wikidot.com/> | supporting |

The canonical source is the one the chart's node table names for this variant. Supporting references are matched by subject and were not attributed to it by the chart.

## Collected files

Collected documents live in this folder. Log each one here as it lands, so the
file's origin can be checked later. Provenance values are defined in
[`_dev/CONVENTIONS.md`](../../../_dev/CONVENTIONS.md).

| File | Kind | Source | Retrieved | Provenance |
| --- | --- | --- | --- | --- |
| _nothing collected yet_ | | | | |
